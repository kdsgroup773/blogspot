...
async function fetchAndDisplayFeed(feedUrl, sourceText, displayContainer, isSingleFeed = false, optionId = '') {
    ...
    try {
        ...
        if (isSingleFeed) {
            ...
            displayContainer.innerHTML += `<p style="color: orange;">Could not load '${sourceText}' feed. Error: ${errorMessage}... URL: <<option id=https://kensbookinfo.blogspot.com value=">${feedUrl}</span></p>`;            
        }
    }
}
